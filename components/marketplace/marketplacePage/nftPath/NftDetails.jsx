...
const NftDetailsAndBuy = ({ nftData }) => {
    ...
    const checkIfCanUnhide = async () => {
        try {
            ...
            if (approved === true) {
                const response = await fetch(platformBackend + `decode-hla/${nftData.tokenId}`)
                if (response.ok) {
        ...
        <div className="w-2/3 flex flex-col px-8">
            <div className="border-l-2 p-4 mb-2 rounded-xl w-full ">
                ...
                <div className="flex flex-col pt-4">
                    ...
                    {hlaHidden ? (
                        <div onClick={checkIfCanUnhide} className="flex flex-row justify-between py-4 px-6 w-3/4 font-satoshiMedium text-base rounded-md bg-hiddenHla text-black">
